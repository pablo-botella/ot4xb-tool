...
func (t *Tool) stepDocs(e *entry, cfg json.RawMessage, o Options) error {
	...
	db := ""
	if c.DB != "" {
		if db, err = t.expandPath(e, c.DB); err != nil {
			return err
		}
	} else if db, err = conf.DBPath(); err != nil {
		return err
	}
	if db == "" {
		return fmt.Errorf("docs: no database: give \"db\" here or in the doc tool file")
	}
	...
	} else if srcs, err = conf.Sources(); err != nil {
		return err
	}
	if len(srcs) == 0 {
		return fmt.Errorf("docs: no sources: give \"src\" here or in the doc tool file")
	}
