...
func Build(dbPath string, conf *docconf.Config, log func(string)) ([]Page, error) {
	if conf == nil {
...
// indexes not counted.
func Generate(dbPath, outDir string, conf *docconf.Config, log func(string)) (int, error) {
...
func (p *page) books(conf *docconf.Config) []string {
	var out []string
	for _, t := range p.topics {
...
func (p *page) inBook(conf *docconf.Config, b string) bool {
...
func (p *page) indexed(conf *docconf.Config) bool {
...
func (p *page) matches(f docconf.Filter) bool {
	if f.Kind != "" && f.Kind != "*" && p.indexKind() != f.Kind {
		return false
	}
	return docconf.MatchCategory(f.Category, p.cats())
}
...
// by title.
func (m *model) selectPages(conf *docconf.Config, book string, sec *docconf.Section) []*page {
	if sec.Book != "" {
		book = sec.Book
	}
	var out []*page
...
func dominant(conf *docconf.Config, pages []*page) string {
...
func (m *model) entry(conf *docconf.Config, p *page, dom string) string {
...
func (m *model) renderIndexes(conf *docconf.Config) map[string]string {
