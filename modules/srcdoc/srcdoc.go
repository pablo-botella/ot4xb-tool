...
// Kinds lists the nine topic kinds in presentation order.
...
func HeaderKind(label string) string {
	if label == "class-name" {
		return KindClass
	}
	if kindSet[label] {
...
// Key normalizes an identity for lookup: Xbase++ names (function,
// internal-function, class) are case-insensitive and stored upper-case; C and
// C++ names keep their case; note ids and topic names are lower-case.
func Key(kind, ident string) string {
	ident = strings.TrimSpace(ident)
	switch kind {
	case KindFunction, KindInternalFunction, KindClass:
		return strings.ToUpper(ident)
	case KindNote, KindTopic:
