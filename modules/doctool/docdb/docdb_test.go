...
// TestCfg stores and reads back the configuration blocks: what the collecting
// side writes so the generating side needs no .doc-tool of its own.
func TestCfg(t *testing.T) {
	...
	defer db.Close()
	if _, ok, err := db.Cfg("kinds"); err != nil || ok {
		t.Fatalf("a fresh database has no blocks: ok=%v err=%v", ok, err)
	}
	if err := db.SetCfg("kinds", `[{"name":"function"}]`); err != nil {
		t.Fatal(err)
	}
	v, ok, err := db.Cfg("kinds")
	if err != nil || !ok || v != `[{"name":"function"}]` {
		t.Fatalf("got %q ok=%v err=%v", v, ok, err)
	}
	// a second compile replaces the block, it does not fail on the key
	if err := db.SetCfg("kinds", `[]`); err != nil {
		t.Fatal(err)
	}
	if v, _, _ := db.Cfg("kinds"); v != `[]` {
