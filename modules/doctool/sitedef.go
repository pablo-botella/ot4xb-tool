...
type Site struct {
	...
	// TemplatePage and TemplateIndex name the files inside Templates ("" =
	// the built-in names). They are what lets one templates folder serve
	// several sites: page.html and index.html can only belong to one.
...
func (c *Site) OutDir() string { return c.siteAbs(c.Out) }

// DocToolPath is the .doc-tool file, absolute ("" when none).
func (c *Site) DocToolPath() string {
	if c.DocTool == "" {
		return ""
	}
	return c.siteAbs(c.DocTool)
}

// DBPath is the database, absolute ("" when none).
func (c *Site) DBPath() string {
	if c.DB == "" {
		return ""
	}
	return c.siteAbs(c.DB)
}

// AssetsDir is the assets folder, absolute ("" when none).
func (c *Site) AssetsDir() string {
	if c.Assets == "" {
		return ""
	}
	return c.siteAbs(c.Assets)
}

// TemplatesDir is the templates folder, absolute ("" when none).
func (c *Site) TemplatesDir() string {
	if c.Templates == "" {
		return ""
	}
	return c.siteAbs(c.Templates)
}

func (c *Site) siteAbs(p string) string {
	if filepath.IsAbs(p) {
